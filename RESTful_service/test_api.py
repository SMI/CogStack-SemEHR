...
url_prefix = 'http://localhost:${port}/api/search_anns/C0205076/?passphrase=${passphrase}'
url_prefix = 'http://localhost:${port}/api/search_anns/x/?'

# Password?
url = 'http://localhost:8000/api/need_passphrase/'
print('Need password? %s' % urllib.request.urlopen(urllib.request.Request(url)).read().decode())
#url = 'http://localhost:8485/api/check_phrase/${passphrase}/'
#print(urllib.request.urlopen(urllib.request.Request(url)).read().decode())
